package org.lenchan139.justbrowser.Adapter.arraypageradapter;
...
/**
 * ViewPager browseAdapter that handles data and fragments' states.
 * If there are large number of pages or need to save fragments' and items' state, it is recommended to use this browseAdapter.
...
public abstract class ArrayFragmentStatePagerAdapter<T> extends ArrayPagerAdapter<T> {
    ...
    public boolean isViewFromObject(View view, Object object) {
        return object == view.getTag(avpa_view_tag_key);
    }
    ...
    private Fragment findFragmentByItem(Object item) {
        ...
        for (Fragment fragment : fragments) {
            if (fragment != null) {
                ...
                if (view != null && item == view.getTag(avpa_view_tag_key)) {
                    return fragment;
                }
            }
        }
    ...
    private void setTags(List<IdentifiedItem<T>> items) {
        for (int i = 0; i < mFragments.size(); i++) {
            ...
            if (fragment != null && fragment.getView() != null) {
                fragment.getView().setTag(avpa_view_tag_key, items.get(i));
            }
        }
    }
}
